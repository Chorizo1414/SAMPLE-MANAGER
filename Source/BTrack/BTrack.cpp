...
{
    // {* NUEVO: Interpolación lineal personalizada. Adiós libsamplerate! *}
    double ratio = (double)onsetDFBufferSize / 512.0;
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        double val1 = onsetDF[index];
        double val2 = (index + 1 < onsetDFBufferSize) ? onsetDF[index + 1] : 0.0;

        resampledOnsetDF[i] = val1 + fraction * (val2 - val1);
    }
}
...
void BTrack::adaptiveThreshold(std::vector<double>& x)
{
    ...
    // {* CORRECCIÓN: Arreglo dinámico compatible con Visual Studio *}
...
void BTrack::updateCumulativeScore(double onsetDetectionFunctionSample)
{
    ...
    // {* CORRECCIÓN: Usamos std::vector en lugar de arreglo variable *}
    std::vector<double> logGaussianTransitionWeighting(windowSize);
    createLogGaussianTransitionWeighting(&logGaussianTransitionWeighting[0], windowSize, beatPeriod);

    // calculate the new cumulative score value
    double cumulativeScoreValue = calculateNewCumulativeScoreValue(cumulativeScore, &logGaussianTransitionWeighting[0], windowStart, windowEnd, onsetDetectionFunctionSample, alpha);
...
void BTrack::predictBeat()
{
    ...
    // {* CORRECCIÓN: Vectores dinámicos en lugar de arreglos *}
    ...
    // {* CORRECCIÓN: Vector dinámico *}
    std::vector<double> logGaussianTransitionWeighting(pastWindowSize);
    createLogGaussianTransitionWeighting(&logGaussianTransitionWeighting[0], pastWindowSize, beatPeriod);

    for (int i = onsetDFBufferSize; i < (onsetDFBufferSize + beatExpectationWindowSize); i++)
    {
        futureCumulativeScore[i] = calculateNewCumulativeScoreValue(futureCumulativeScore, &logGaussianTransitionWeighting[0], startIndex, endIndex, 0.0, 1.0);
